...
import de.immobilienscout24.rest.schema.common._1.ApiSearchData;
...
import de.is24.gif.ida.converter.is24ToIda.validators.RealestateValidator;
...
public abstract class RealestateTransformer<T extends RealEstate, A extends Object> extends BasicTransformer<T, A> {
	...
	protected void updateRealestateGenericFields(T realestate, SalesforceTransformContext context, Ch__Angebot__c angebot) {
		...
		if (new DuckType(realestate).quacksLikeA(EnergyCertificateFacade.class)) {
			energyCertificateTransformer.updateEnergyCertificate(DuckType.coerce(realestate).to(EnergyCertificateFacade.class), angebot);
		}
	}

	private void updateCountry(T realestate, Ch__Angebot__c angebot, SalesforceTransformContext context) {

		Wgs84Address address = realestate.getAddress();

		if (address != null) {
			CountryRegion internationalCountryRegion = address.getInternationalCountryRegion();
			...
			if (internationalCountryRegion != null) {
				...
				if (country != null) {
					...
					angebot.setCh__Land__c(new Countries().getCountryNameByCode(country.name()));
				}
	...
	private void updateContact(T realestate, Ch__Angebot__c angebot) {
		de.immobilienscout24.rest.schema.offer.realestates._1.RealEstate.Contact realestateContact = realestate.getContact();
		if (realestateContact != null) {
			String id = realestateContact.getId().toString();

			LOG.info("Setting contact with id " + id + " to offer " + angebot.getName());
	...
	public void updateCourtage(T realestate, Ch__Angebot__c angebot) {
		RealestateFacade facade = DuckType.coerce(realestate).to(RealestateFacade.class);
	...
	private void updateSetImmoType(T realestate, Ch__Angebot__c angebot, SalesforceTransformContext context) {
		String immotypeName = getImmotypeName(realestate);
		String recordTypeId = context.getRecordTypeMappingAngebot().get(immotypeName);
	...
	public final boolean isAbroad(T realestate) {
		...
		if (address != null && address.getGeoHierarchy() != null) {
			String fullGeoCodeId = address.getGeoHierarchy().getCountry().getFullGeoCodeId();
			return !("1276".equals(fullGeoCodeId) || fullGeoCodeId == null || fullGeoCodeId.isEmpty());
	...
	public void updateApiSearchData(RealEstate realestate, Ch__Angebot__c angebot) {
