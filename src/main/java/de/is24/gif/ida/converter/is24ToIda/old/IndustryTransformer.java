...
import de.is24.gif.ida.converter.is24ToIda.helpers.IndustryTypeHelper;
...
public class IndustryTransformer extends CommercialTransformer<Industry> {
	protected IndustryTypeHelper industryTypeHelper = new IndustryTypeHelper();

	@Override
	protected Ch__Angebot__c doTransform(Industry industry, ITransformContext context) {

		Ch__Angebot__c angebot = new Ch__Angebot__c();
		updateRealestateGenericFields(industry, (SalesforceTransformContext) context, angebot);

		CommercialFacade facade = DuckType.coerce(industry).to(CommercialFacade.class);
