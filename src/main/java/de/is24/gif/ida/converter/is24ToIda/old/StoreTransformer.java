...
import de.is24.gif.ida.converter.is24ToIda.helpers.LocationClassificationTypeHelper;
import de.is24.gif.ida.converter.is24ToIda.helpers.OptionalTypeHelper;
import de.is24.gif.ida.converter.is24ToIda.helpers.StoreTypeHelper;
import de.is24.gif.ida.converter.is24ToIda.helpers.SupplyTypeHelper;
...
public class StoreTransformer extends CommercialTransformer<Store> {
	...
	protected LocationClassificationTypeHelper locationClassificationTypeHelper = new LocationClassificationTypeHelper();
	protected SupplyTypeHelper supplyTypeHelper = new SupplyTypeHelper();

	@Override
	protected Ch__Angebot__c doTransform(Store store, ITransformContext context) {

		Ch__Angebot__c angebot = new Ch__Angebot__c();
		updateRealestateGenericFields(store, (SalesforceTransformContext) context, angebot);

		CommercialFacade facade = DuckType.coerce(store).to(CommercialFacade.class);
